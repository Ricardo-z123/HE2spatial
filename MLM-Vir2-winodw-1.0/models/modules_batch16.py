...
class ImageEncoder_Virchow2(nn.Module):
    ...
    Input:  x [B, 3, 224, 224]   H&E spot patches after ToTensor(), values in [0, 1]
    Output: x [B, 1280]          patch-mean Virchow2 features per spot
    """
    def __init__(self, cache_dir="/root/autodl-tmp/huggingface/hub", forward_batch_size=16):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for x_chunk in x.split(self.forward_batch_size, dim=0):
            with torch.inference_mode(), torch.autocast(
                device_type=x.device.type,
                dtype=torch.float16,
                enabled=x.is_cuda,
            ):
                output = self.model(x_chunk)  # [b, 261, 1280]

            patch_tokens = output[:, 5:]  # [b, 256, 1280], skip class + register tokens
            features.append(patch_tokens.mean(1))  # [b, 1280]

        return torch.cat(features, dim=0)  # [B, 1280]
