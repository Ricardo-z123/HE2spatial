...
from modules import ImageEncoder, ProjectionHead, ImageEncoder_ViT, ImageEncoder_ViT_L, ImageEncoder_CLIP, \
    ImageEncoder_resnet101, ImageEncoder_resnet152, SpotEncoder_v1
...
class CLIPModel(nn.Module):
    def __init__(
        ...
        self.spot_encoder = SpotEncoder_v1(spot_dim=spot_embedding)  ## 修改1
        self.image_projection = ProjectionHead(embedding_dim=image_embedding)  # aka the input dim, 2048 for resnet50
        self.spot_projection = ProjectionHead(embedding_dim=spot_embedding)  # 6419 shared hvgs
    ...
    def forward(self, batch):
        ...
        spot_features = self.spot_encoder(      ## 修改2
            batch["reduced_expression"],
            batch["positions"]
        )
        ...
        logits = (spot_embeddings @ image_embeddings.T) / self.temperature
        targets = torch.arange(logits.shape[0]).to(logits.device)  # arange

        spot_loss = F.cross_entropy(logits, targets)
        images_loss = F.cross_entropy(logits.T, targets)

        loss = (images_loss + spot_loss) / 2.0
        return loss.mean()
...
class CLIPModel_CLIP(nn.Module):
    ...
    def forward(self, batch):
        # Getting Image and spot Features
        image_features = self.image_encoder(batch["image"])
        spot_features = self.spot_encoder(  ## 修改3
            batch["reduced_expression"],
            batch["positions"]
        )
        # spot_features = batch["reduced_expression"]
...
class CLIPModel_ViT_L(nn.Module):
    ...
    def forward(self, batch):
        # Getting Image and spot Features
        image_features = self.image_encoder(batch["image"])
        spot_features = self.spot_encoder(  ## 修改4
            batch["reduced_expression"],
            batch["positions"]
        )
        # spot_features = batch["reduced_expression"]
...
class CLIPModel_resnet152(nn.Module):
    ...
    def forward(self, batch):
        # Getting Image and spot Features
        image_features = self.image_encoder(batch["image"])
        spot_features = self.spot_encoder(  ##
            batch["reduced_expression"],
            batch["positions"]
        )
        # spot_features = batch["reduced_expression"]
...
if __name__ == '__main__':
    ...
    reduced_expression = torch.randn(batch_size, CFG.spot_embedding)
    positions = torch.randint(0, 100, size=(batch_size, 2))
    batch = {
        'image': images,
