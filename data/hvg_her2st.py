...
def her2_hvg_selection_and_pooling(adata_list, n_top_genes=1000):
    ...
    with open('/root/autodl-tmp/HER2ST-MLM/data/her2_hvgs_intersection.pickle', 'wb') as handle:
        pickle.dump(hvg_intersection, handle, protocol=pickle.HIGHEST_PROTOCOL)
    with open('/root/autodl-tmp/HER2ST-MLM/data/her2_hvgs_union.pickle', 'wb') as handle:
        pickle.dump(hvg_union, handle, protocol=pickle.HIGHEST_PROTOCOL)

    # Add all the HVGs

    gene_list_path = "/root/autodl-tmp/HER2ST-MLM/data/her_hvg_cut_1000.npy"
...
for i in range(len(filtered_mtx)):
    pathset = f"/root/autodl-tmp/HER2ST-MLM/data/filtered_expression_matrices/her2/{names[i]}"
    if not (os.path.exists(pathset)):
        os.makedirs(pathset)

    np.save(f"/root/autodl-tmp/HER2ST-MLM/data/filtered_expression_matrices/her2/{names[i]}/hvg_matrix_plusmarkers.npy", filtered_mtx[i])  # save [785, spots] raw count (un-normalized)
...
for i, mtx in enumerate(filtered_mtx):  # 6 slide，each [785, spots]. each slide raw count, filtered to 785 HVGs.
    ...
    if not os.path.exists(pathset):
        os.makedirs(pathset)
    np.save(f"/root/autodl-tmp/HER2ST-MLM/data/preprocessed_expression_matrices/her2st/{names[i]}/preprocessed_matrix.npy",
