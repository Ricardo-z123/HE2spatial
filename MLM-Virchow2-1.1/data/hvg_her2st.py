...
adata_list = [sc.AnnData(pd.read_csv(f"/root/autodl-tmp/Her2st/data/ST-cnts/{name}.tsv",
                                     sep='\t', index_col=0)) for name in names]
#
...
for i in range(len(filtered_mtx)):
    ...
    np.save(f"/root/autodl-tmp/HER2ST-MLM/data/filtered_expression_matrices/her2/{names[i]}/hvg_matrix_plusmarkers.npy", filtered_mtx[i])
