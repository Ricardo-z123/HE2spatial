...
class MaskedEncoder(nn.Module):
    ...
    Spot and image branches share this class; only dim differs:
        ...
        image branch: dim = 2560  (Virchow2 concat=CLS+patch-mean per spot); also 1024/2048 for DenseNet/ResNet
    enc_dim = dec_dim = dim  (no encoder->decoder projection; "MAE-lite").

    Shapes below use the ACTUAL training run (window mode):
        B = 8 windows per batch,  L = 100 spots per window,  dim = 785,
        mask_ratio = 0.5  ->  len_keep = 50 kept,  50 masked.
    """
    ...
    def __init__(self, dim, enc_depth, dec_depth, num_heads,
        ...
        super().__init__()
        self.mask_ratio = mask_ratio                          # fraction of spots to drop (0.5)
    ...
    def random_masking(self, x):
        ...
        Input:
          x [B, L, D]   e.g. [8, 100, 785]   spot sequence (already + pos_embed)
        Output:
          x_masked    [B, len_keep, D]   e.g. [8, 50, 785]   the KEPT spots, in SHUFFLED order
          mask_label  [B, L]             e.g. [8, 100]        0 = kept, 1 = masked, in ORIGINAL spot order
          ids_restore [B, L]             e.g. [8, 100]        inverse permutation: used later to undo the shuffle
        """
        B, L, D = x.shape
        len_keep = int(L * (1 - self.mask_ratio))  # spots to keep, e.g. L=100,ratio=0.5 -> 50 keep / 50 masked

        noise = torch.rand(B, L, device=x.device)  # one random number in [0,1) per spot, per sample -> [B, L]

        ids_shuffle = torch.argsort(noise, dim=1)  # shape: [8, 100] sort noise: gives a RANDOM permutation of spot ids 0..L-1 (small noise first)
        ids_restore = torch.argsort(ids_shuffle, dim=1)  # shape : [8, 100] argsort of a permutation = its INVERSE (maps shuffled position -> original position)

        ids_keep = ids_shuffle[:, :len_keep]       # [B, 50] keep first len_keep 50 ids_shuffle positions as x_kept (random order）
        # ids_keep [B,50] --unsqueeze(-1)--> [B,50,1] --repeat(1,1,785)--> [B,50,785]
        # x[8,100,785] --> x_masked [8,50,785].
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))  # gather those kept spots' features -> [B, 50, 785] (random order)

        mask_label = torch.ones([B, L], device=x.device)  # [8, 100], build the 0/1 label: start all 1 (= masked)
        mask_label[:, :len_keep] = 0         # first len_keep 50 positions are kept -> 0 (NOTE: still in shuffled order here)
        mask_label = torch.gather(mask_label, dim=1, index=ids_restore)  # unshuffle the label back to ORIGINAL spot order -> [B, L]

        return x_masked, mask_label, ids_restore  # [8, 50, 785], [8, 100], [8, 100], [8, 100].
    ...
    def forward_encoder(self, x, coords):
        ...
        (Encoder runs on 50 spots, not 100 -> this is MAE's compute saving.)

        Input:
          x      [B, L, dim]   e.g. [8, 100, 785]   spot features
          coords [B, L, 2]        e.g. [8, 100, 2]     integer (x, y) grid coords per spot
        Output:
          x_kept (encoded) [B, len_keep, dim]   e.g. [8, 50, 785]   encoder output for kept spots (shuffled order)
          mask_label       [B, L]                  e.g. [8, 100]       0=keep, 1=masked (original order)
          ids_restore      [B, L]                  e.g. [8, 100]       to restore shuffled -> original later
        """
        x = x + self.pos_embed(coords)  # add learnable spatial PE per spot -> [B, L, dim]
        x_kept, mask_label, ids_restore = self.random_masking(x)  # drop spots -> x_kept [B,50,dim]!!; mask_label/ids_restore [B,100]
        for blk in self.encoder_blocks:
            x_kept = blk(x_kept)        # Transformer block, shape unchanged -> [B, 50, dim]
        x_kept = self.encoder_norm(x_kept)  # final LayerNorm -> x_kept [B, 50, dim]
    ...
    def forward_decoder(self, x, ids_restore, coords):
        ...
        Input:
          x     [B, len_keep, dim]   e.g. [8, 50, 785]   encoder output for kept spots (SHUFFLED order)
          ids_restore [B, L]                  e.g. [8, 100]       restore index from forward_encoder
          coords      [B, L, 2]               e.g. [8, 100, 2]    original-order spot coords
        Output:
          pred [B, L, dim]   e.g. [8, 100, 785]   reconstructed features for ALL spots, original order
        """
        mask_tokens = self.mask_token.repeat(x.shape[0], ids_restore.shape[1] - x.shape[1], 1)  # one mask_token per dropped spot -> [8, 50, 785]
    ...
    def forward_loss(self, target, pred, mask_label):
        ...
        Input:
          target     [B, L, dim]   e.g. [8, 100, 785]   ORIGINAL spot features (the reconstruction goal)
          pred       [B, L, dim]   e.g. [8, 100, 785]   decoder output
          mask_label [B, L]           e.g. [8, 100]        0=keep, 1=masked
        Output:
          loss scalar   mean MSE over masked spots only
        """
        loss = (pred - target) ** 2  # [B, L, dim] squared error per gene/feature
        loss = loss.mean(dim=-1)  # per-spot mean over feature dim -> [B, L], [8, 100], mask_label [8, 100]
    ...
    def forward(self, x, coords):
        ...
        Input:
          x      [B, L, dim]   e.g. [8, 100, 785]   raw spot features (gene or image)
          coords [B, L, 2]        e.g. [8, 100, 2]     integer (x, y) coords per spot
        Output:
          loss       scalar tensor                      mean MSE over masked spots
          pred       [B, L, dim]   e.g. [8, 100, 785]   reconstruction for all spots
          mask_label [B, L]           e.g. [8, 100]        0=keep, 1=masked
        """
        target = x.clone()  # snapshot the RAW features as the reconstruction target, BEFORE any pos_embed is added
        x_kept_encoded, mask_label, ids_restore = self.forward_encoder(x, coords)  # encode kept spots only -> [B, 50, dim]
...
## Example for encoder + decoder:
    ...
    #
    #  forward(x [8,100,785], coords [8,100,2])
    #    target = x.clone()                                  # [8,100,785] raw features, BEFORE pos_embed (reconstruction target)
    #    │
    #    ├─ forward_encoder:
    #    │    x + pos_embed                                  # [8,100,785]
    #    │    random_masking  -> x_kept [8,50,785] (shuffled, drop 50)
    #    │                       mask_label [8,100], ids_restore [8,100]
    #    │    4x encoder block + norm                        # x_kept [8,50,785]  (still shuffle order) (only 50 go through encoder)
    #    │
    #    ├─ forward_decoder(x_kept, ids_restore, coords):
    #    │    mask_tokens = mask_token.repeat                # [8,50,785] (50 placeholders)
    #    │    cat(x_kept, mask_tokens)                       # [8,100,785] (shuffled: 50 real + 50 placeholder)
    #    │    gather(ids_restore)                            # [8,100,785] (back to ORIGINAL order)
    #    │    + pos_embed                                    # [8,100,785]
    #    │    2x decoder block + norm + Linear(decoder_pred) # pred [8,100,785] (reconstruct ALL spots)
    #    │
    #    └─ forward_loss(target, pred, mask_label):
    #         (pred-target)^2 -> mean over genes -> * mask_label (keep masked only) -> mean
    #                                                         # loss = scalar (MSE on the 50 masked spots)
    #
    #    return loss, pred, mask_label