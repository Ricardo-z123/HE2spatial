...
class ImageEncoder_Virchow2(nn.Module):
    """
    Frozen Virchow2 backbone (a pathology ViT foundation model, paige-ai/Virchow2).
    ...
    Output: x [B, 1280]          one 1280-d feature per spot (mean of the 256 patch tokens).

    NOTE (differs from official usage): the official Virchow2 embedding is
    cat(class_token, patch_tokens.mean) -> 2560-d. Here we keep ONLY patch_tokens.mean
    -> 1280-d (class token dropped). Self-consistent with dim=1280 downstream, but
    not the model authors' recommended 2560-d embedding.
    """
    def __init__(self, cache_dir="/root/autodl-tmp/huggingface/hub"):
        super().__init__()
    ...
    def forward(self, x):                        # x [B, 3, 224, 224] H&E spot patches, values in [0, 1]
        ...
        patch_tokens = output[:, 5:]             # keep only the 256 patch tokens -> [B, 256, 1280]
                                                 ...
                                                 # 256 = the 224x224 image cut into a 16x16 grid of 14x14-px sub-patches
        x = patch_tokens.mean(1)                 # mean-pool over the 256 tokens (dim 1) -> [B, 1280] per-spot feature
        return x      # [B, 1280] frozen Virchow2 feature, one vector per spot
