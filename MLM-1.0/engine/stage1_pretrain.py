...
from models.modules import ImageEncoder, ImageEncoder_Resnet
...
class Stage1Model(nn.Module):
    ...
    Image branch: H&E patches → frozen DenseNet/ResNet → features → MaskedEncoder.
    """
    ...
    def forward(self, batch):
        ...
        if self.branch == 'spot':
            ...
            patches = batch['patches'][0]  # drop batch dim → [L, 3, 224, 224] for DenseNet/ResNet
            with torch.no_grad():  # image_encoder is frozen, no grad needed
                features = self.image_encoder(patches)  # [L, 1024] (DenseNet) or [L, 2048] (ResNet50)
