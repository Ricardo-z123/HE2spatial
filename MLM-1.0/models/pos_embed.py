...
class PosEmbed(nn.Module):
    ...
    Each spot has integer (x, y) coordinates on the slide grid (HER2ST raw coords are
    bounded by ~65536 on each axis, hence vocab=65536 — same value used in baseline
    mclSTExp model.py L204–205).
    ...
    def __init__(self, dim):
        super().__init__()
        self.x_embed = nn.Embedding(65536, dim)  # vocab covers full HER2ST x-coord range # max 30
        self.y_embed = nn.Embedding(65536, dim)  # vocab covers full HER2ST y-coord range # max 30
