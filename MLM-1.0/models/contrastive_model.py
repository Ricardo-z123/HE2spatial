...
from models.modules import ImageEncoder, ImageEncoder_Resnet, ProjectionHead
...
class ContrastiveModel(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg['encoder_name'] == 'densenet121':
            ...
            self.image_cnn = ImageEncoder_Resnet()  # baseline L207 — outputs [N, 1024]
        else:
    ...
    def forward(self, batch):
        ...
        """
        # ===== Image path =====
        image_features = self.image_cnn(batch["image"])           # [N, 1024], baseline L226
        image_features = image_features.unsqueeze(dim=0)          # [1, N, 1024], add slide-batch dim for transformer
        coords = batch["position"].long().unsqueeze(dim=0)        # [1, N, 2]
        image_features = self.image_encoder.encode(image_features, coords)  # [1, N, 1024], extra Transformer over CNN feats
        image_features = image_features.squeeze(dim=0)            # [N, 1024]
