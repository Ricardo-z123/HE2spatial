...
for i in range(len(filtered_mtx)):
    pathset = f"/root/autodl-tmp/mclSTExp/data/filtered_expression_matrices/her2/{names[i]}"
    if not (os.path.exists(pathset)):
        os.makedirs(pathset)

    np.save(f"/root/autodl-tmp/mclSTExp/data/filtered_expression_matrices/her2/{names[i]}/hvg_matrix_plusmarkers.npy", filtered_mtx[i])
...
def her2_pool_gene_list(adata_list, n_top_genes=1000):
    ...
    for adata in adata_list:
        adata.var_names_make_unique()
        filtered_exp_mtxs.append(adata[:, gene_list].X.T.toarray())   #[785, num_spots]
    return filtered_exp_mtxs
...
for i, mtx in enumerate(filtered_mtx):
    ...
    pathset = f"/root/autodl-tmp/mclSTExp/data/preprocessed_expression_matrices/her2st/{names[i]}"
    if not os.path.exists(pathset):
        os.makedirs(pathset)
    np.save(f"/root/autodl-tmp/mclSTExp/data/preprocessed_expression_matrices/her2st/{names[i]}/preprocessed_matrix.npy",
